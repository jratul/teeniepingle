...
**Data layer (`src/app/contant.ts`, note: not "constant")**
A single ~1600-line static data file is the source of truth for all content:
...
- Each `Ping.img` field maps to an image file at `public/images/pings/{img}.webp`
...
- `page.tsx` owns local component state for the search text box and the derived `filteredPingInfo`, recomputed in a `useEffect` whenever `filterGroup` or `searchName` changes (filters `pingData` by checked type + name substring match).

**Component flow**
`page.tsx` → `Filter` (collapsible toggle wrapping `FilterContent`, which renders `CheckBox` items bound to the Zustand store) and one `SeasonFrame` per season with results → `SeasonFrame` renders a grid of `PingItem` (image + name, animated in via `framer-motion`'s `whileInView`) → clicking a `PingItem` opens `PingDialog` (rendered through `Portal` into the `#portal` div defined in `layout.tsx`) showing a YouTube embed, character line, and `PingTable` (full character detail table).

**Styling**
Uses `@emotion/styled` (not Tailwind, not CSS Modules) throughout — all styled components are defined inline at the top of each file. `next.config.mjs` sets `compiler.styledComponents: true`, which is a no-op for this codebase since it uses Emotion, not `styled-components`.

**Fonts**
Pretendard variable font is self-hosted via `next/font/local` in `layout.tsx` (no Google Fonts CDN).
